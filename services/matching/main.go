...
// StartGRPCServer starts the Matching Service gRPC server
func StartGRPCServer(lc fx.Lifecycle, log *zap.Logger, cfg *config.Config, handler *MatchingHandler) {
	...
	lc.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			...
			log.Info("Starting Matching Service gRPC server", zap.String("port", cfg.Port))
			go func() {
		...
		OnStop: func(ctx context.Context) error {
			log.Info("Stopping Matching Service gRPC server gracefully...")
