...
// StartGRPCServer starts the Trip Service gRPC server
func StartGRPCServer(lc fx.Lifecycle, log *zap.Logger, cfg *config.Config, handler *TripHandler) {
	...
	lc.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			...
			log.Info("Starting Trip Service gRPC server", zap.String("port", cfg.Port))
			go func() {
		...
		OnStop: func(ctx context.Context) error {
			log.Info("Stopping Trip Service gRPC server gracefully...")
