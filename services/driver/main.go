...
// StartGRPCServer starts the Driver Service gRPC server
func StartGRPCServer(lc fx.Lifecycle, log *zap.Logger, cfg *config.Config, handler *DriverHandler) {
	...
	lc.Append(fx.Hook{
		OnStart: func(ctx context.Context) error {
			...
			log.Info("Starting Driver Service gRPC server", zap.String("port", cfg.Port))
			go func() {
		...
		OnStop: func(ctx context.Context) error {
			log.Info("Stopping Driver Service gRPC server gracefully...")
